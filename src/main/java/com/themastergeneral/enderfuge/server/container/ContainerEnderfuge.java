package com.themastergeneral.enderfuge.server.container;
...
public class ContainerEnderfuge extends Container {
	...
	public ContainerEnderfuge(InventoryPlayer playerInventory,
			IInventory furnaceInventory) {
		this.tileFurnace = furnaceInventory;
		this.addSlotToContainer(new Slot(furnaceInventory, 0, 56, 17));
		this.addSlotToContainer(new SlotEnderfugeFuel(furnaceInventory, 1, 56,
				53));
		this.addSlotToContainer(new SlotFurnaceOutput(playerInventory.player,
				furnaceInventory, 2, 116, 35));
		...
		for (int k = 0; k < 9; ++k) {
			this.addSlotToContainer(new Slot(playerInventory, k, 8 + k * 18,
					142));
		}
	}
	...
	public void detectAndSendChanges() {
		...
		for (int i = 0; i < this.listeners.size(); ++i) {
			...
			if (this.totalCookTime != this.tileFurnace.getField(3)) {
				icontainerlistener.sendWindowProperty(this, 3,
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index == 2) {
				...
				if (!EnderfugeRecipes.instance().getSmeltingResult(itemstack1)
						.isEmpty()) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false)) {
						return ItemStack.EMPTY;
					}
				} else if (TEEnderfuge.isItemFuel(itemstack1)) {
